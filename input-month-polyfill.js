(function () {
    ...
    if (monthInputs.length == 0) {
        // there are no month type input elements; no need for polyfill
        return true;
    }
    ...
    monthInputs.forEach(function (monthInput) {
        monthInput.setAttribute("hidden","hidden");
        var initial = monthInput.value,
    ...
    function updateValue(input) {
        var date, newValue, monthDisplay;
    ...
    document.addEventListener('change', function (event) {
        ...
        if (changed.monthInputTarget && changed.monthInputType) {
            ...
            if (!updateValue(changed.monthInputTarget)) {
                ...
                changed.monthInputTarget.dataset[changed.monthInputType] = changed.dataset
                    .storedValue;
                changed.focus();
            } else {
